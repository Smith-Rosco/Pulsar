...
|---|---|---|---|
| **短期（1–2 迭代）** | 方向一：手势细节 + **多语言扩展** | 手势细节 ✅ 全落地；多语言 ❌ 仍 EN + zh-CN（`Resources/` 仅 `Strings.resx` + `Strings.zh-CN.resx`，无 zh-TW / ja） | 🟡 **半完成** |
...
| 2 | **多语言扩展**（zh-TW / ja） | 短期路线 | 中高 —— StarPie 四语热切换 | 复用 `ILocalizationService`，按 `localization-infrastructure` spec 扩 resx |
...
> **2026-09-03 后续处理**：前四项已在本报告产出后同轮修复完毕，仅末项待办。
...
| **`Fan` 分页语义未固化** | 子环 + 分页叠加的语义只在代码里 | ⬜ **待办** — 建议补 ADR |

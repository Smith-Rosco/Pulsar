...
SystemCommand 是 Pulsar 的内部系统控制插件，提供对 Pulsar 自身功能的访问（如打开设置、快速添加配置等）。
...
### `pulsar.system.open_settings` - 打开设置
...
### `pulsar.system.quick_add_profile` - 快速添加配置
...
1. **不可禁用**: 作为核心插件，SystemCommand 无法被禁用
2. **UI 依赖**: 需要 WPF UI 线程，不能在无 UI 环境中使用
3. **上下文感知**: `quick_add_profile` 依赖 `PulsarContext.TargetProcessName`

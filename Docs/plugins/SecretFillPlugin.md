...
The Secret Fill runtime is split into four layers:

1. `SecretFillPlugin`
   - exposes plugin metadata
   - preserves action compatibility (`fill` + `inject`)
   - delegates execution to `ISecretFillExecutionService`
2. `SecretFillExecutionService`
...
3. Shared Secret Fill domain/application services
   - `ISecretStore`
   - `ISecretProtector`
...
4. Windows adapters
   ...
   - input/focus helper adapters under `Plugins/Core/SecretFill/Services/Input/`

## Injection Policy

Secret Fill credential fill is explicitly SendKeys-first.
...
UI Automation helpers may still exist in infrastructure for non-Secret-Fill scenarios, but Secret Fill credential fill does not depend on UIA-first behavior.

## Shared Secret Management

Runtime and settings flows use the same Secret Fill secret contracts:
...
Secret Fill execution distinguishes these stages internally:
...
Secret Fill validation is covered by layered tests for:

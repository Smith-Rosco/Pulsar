...
| Runtime-installed plugin never activates (`Profiles.json` plugins empty) | Discovery is a startup snapshot; install-time `GrantPermissionsAsync` silently WARNs "unknown plugin". Install flow: `RefreshDiscoveryAsync` → grant → activate. | [PLUGIN_RUNTIME_INSTALL_UNINSTALL_PITFALLS.md](./Docs/lessons/PLUGIN_RUNTIME_INSTALL_UNINSTALL_PITFALLS.md) |
| Runtime uninstall/overwrite-install fails (`IOException` / "already installed") | Plugin DLLs stay locked: collectible ALC is loaded but never unloaded. Uninstall: revoke permissions → `DeactivatePluginAsync` (runs the full teardown incl. ALC unload) → delete with retry. Husk dirs (no manifest) count as NOT installed. | [PLUGIN_RUNTIME_INSTALL_UNINSTALL_PITFALLS.md](./Docs/lessons/PLUGIN_RUNTIME_INSTALL_UNINSTALL_PITFALLS.md) |
| External plugin's ambient contributions (renderers) vanish after restart | Discovery ≠ activation. External plugins are lazily activated, so `OnEnableAsync`-registered contributions silently drop on restart. `AppStartupCoordinator` must activate enabled external plugins right after `DiscoverDeferredAsync`. | [PLUGIN_RUNTIME_INSTALL_UNINSTALL_PITFALLS.md](./Docs/lessons/PLUGIN_RUNTIME_INSTALL_UNINSTALL_PITFALLS.md) |
| Uninstall "Access to path … denied" despite successful `Unload()` | Collectible ALC teardown is GC-driven; `Unload()` only initiates it, so DLL locks persist until GC collects the context. Force `GC.Collect()` + `WaitForPendingFinalizers` + `GC.Collect()` after `TryUnloadExternalContext` before deleting the directory. | [PLUGIN_RUNTIME_INSTALL_UNINSTALL_PITFALLS.md](./Docs/lessons/PLUGIN_RUNTIME_INSTALL_UNINSTALL_PITFALLS.md) |
| Uninstall still fails after forcing GC | `PluginDescriptor.ImplementationType` is a `Type` from the collectible ALC; any holder (e.g. `PluginManagerViewModel`'s descriptor list) pins the context. `DeactivatePluginAsync` must null `descriptor.ImplementationType` (now `Type? { set; }`) before `RemoveDescriptor`. | [PLUGIN_RUNTIME_INSTALL_UNINSTALL_PITFALLS.md](./Docs/lessons/PLUGIN_RUNTIME_INSTALL_UNINSTALL_PITFALLS.md) |
...
| Cross-session working memory | `Docs/journal/` — one file per day via the `session-journal` skill; read at session start, append at session end |

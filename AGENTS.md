...
| Add dialog | [Docs/architecture/DIALOG_SYSTEM.md](./Docs/architecture/DIALOG_SYSTEM.md) - **Register a DataTemplate in `Themes/DialogTemplates.xaml` AND a row in `Services/DialogCatalog.cs` (title key / size / buttons / theme), then show it with `ShowCustomAsync(DialogId.X, content)`.** [ADR-033](./Docs/decisions/033-dialog-catalog-single-registration-surface.md) |
| Add/modify settings page | `Services/SettingsPageCatalog.cs` (register; `IsTransient` for on-demand dynamic tabs) + `Services/SettingsPageFactory.cs` (register creator) + resx keys. Transient pages: per-type singleton, clean-leave auto-recycle — [029-settings-transient-pages.md](./Docs/decisions/029-settings-transient-pages.md). Heavy config = transient page / dedicated page, NOT modal (modals = one-shot confirmations & pickers only) |
...
| Historical fix reports (not current truth) | [Docs/archive/](./Docs/archive/) — date-prefixed `YYYY-MM-DD-NAME.md`, bucketed by month (`2026-03/`, …) |
...
- **Add a Dialog**: `IDialogViewModel` in `ViewModels/Dialogs/` → UserControl in `Views/Dialogs/Contents/` → **register a DataTemplate in `Themes/DialogTemplates.xaml`** → **add a row (id + title key + size preset + buttons + theme) in `Services/DialogCatalog.cs`** → show it with `DialogService.ShowCustomAsync(DialogId.X, content)`. The three legs are guarded by `DialogCatalogTests` / `DialogTemplateRegistrationTests`. Deep dive: [Docs/architecture/DIALOG_SYSTEM.md](./Docs/architecture/DIALOG_SYSTEM.md).

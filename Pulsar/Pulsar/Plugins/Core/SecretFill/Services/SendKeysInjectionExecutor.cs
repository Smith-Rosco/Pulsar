...
namespace Pulsar.Plugins.Core.SecretFill.Services
{
    public class SendKeysInjectionExecutor : IInjectionExecutor
    {
        ...
        public async Task<SecretFillExecutionResult> ExecuteAsync(InjectionPlan plan)
        {
            ...
            try
            {
                    ...
                    switch (step.Type)
                    {
                        case InjectionStepType.HideLauncher:
                            ...
                            catch (Exception ex)
                            {
                                ...
                                return SecretFillExecutionResult.Fail(SecretFillExecutionStage.HideLauncher, "Failed to hide launcher", plan);
                            }
                            break;
                        ...
                        case InjectionStepType.RestoreFocus:
                            try
                            {
                                ...
                                if (!result.Success || !result.VerificationPassed)
                                {
                                    ...
                                    return SecretFillExecutionResult.Fail(SecretFillExecutionStage.FocusRestore, "Failed to restore focus to target window", plan);
                                }
                            }
                            catch (Exception ex)
                            {
                        ...
                        case InjectionStepType.SendText:
                            ...
                            catch (Exception ex)
                            {
                                ...
                                return SecretFillExecutionResult.Fail(SecretFillExecutionStage.Injection, "Credential injection failed during text entry", plan);
                            }
                            break;
                        ...
                        case InjectionStepType.SendKey:
                            ...
                            catch (Exception ex)
                            {
                                ...
                                return SecretFillExecutionResult.Fail(SecretFillExecutionStage.Injection, "Credential injection failed during key entry", plan);
                            }
                            break;
                    }
                }
                ...
                return SecretFillExecutionResult.Ok("Credentials injected successfully", plan);
            }
            catch (OperationCanceledException)
            {
                _logger.LogWarning("[SendKeysInjectionExecutor] Injection timed out for secret {SecretId}", plan.SecretId);
                return SecretFillExecutionResult.Fail(SecretFillExecutionStage.Injection, "Credential injection timed out", plan);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "[SendKeysInjectionExecutor] Unexpected injection failure");
                return SecretFillExecutionResult.Fail(SecretFillExecutionStage.Injection, "Credential injection failed", plan);
            }
        }

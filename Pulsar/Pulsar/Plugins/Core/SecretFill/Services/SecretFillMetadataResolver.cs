...
{
    public class SecretFillMetadataResolver : ISecretFillMetadataResolver
    {
            ...
            IReadOnlyDictionary<Guid, string>? legacyLabels = null)
        {
            ...
            // [UI Debug Mode] Mask SecretFill/secret-bearing display metadata so E2E
            // captures can never leak real credential labels or accounts.
            return new SecretDisplayMetadata(
                secretId,
                DebugSecretFillRedaction.RedactSecretDisplay(label),
                DebugSecretFillRedaction.RedactAccount(payload.Account));
        }
    }
}

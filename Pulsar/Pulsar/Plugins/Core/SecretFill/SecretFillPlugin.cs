// [Path]: Pulsar/Pulsar/Plugins/Core/SecretFill/SecretFillPlugin.cs
...
namespace Pulsar.Plugins.Core.SecretFill
{
    /// <summary>
    /// SecretFill plugin adapter for credential injection.
    /// </summary>
    public class SecretFillPlugin : PluginBase<SecretFillPlugin>, IPluginMetadataProvider, IPluginConfigurable
    {
        private readonly ISecretFillExecutionService _executionService;
        private readonly SecretFillPluginSettings _settings = new();
        ...
        public SecretFillPlugin(
            ILogger<SecretFillPlugin> logger,
            ILocalizationService loc,
            ISecretFillExecutionService executionService)
            : base(logger, loc)
        {
        ...
        {
            SecretFillExecutionResult result = await _executionService.ExecuteAsync(args, context);
            ...
            Logger.LogWarning(
                "[SecretFillPlugin] SecretFill execution failed at stage {Stage}: {Message}",
                result.Stage,
                result.Message);
        ...
        private static PluginErrorCode MapStageToErrorCode(SecretFillExecutionStage stage)
        {
            return stage switch
            {
                SecretFillExecutionStage.Validation => PluginErrorCode.MissingRequiredParameter,
                SecretFillExecutionStage.SecretLookup => PluginErrorCode.NotFound,
                SecretFillExecutionStage.Decryption => PluginErrorCode.InvalidConfiguration,

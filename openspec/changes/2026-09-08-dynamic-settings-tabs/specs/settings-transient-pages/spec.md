...
The system SHALL allow triggering a transient settings page (e.g., via a card's detail button) to append a single navigation entry for that transient page type in the sidebar. Triggering a transient page type that is already open SHALL NOT create an additional entry or instance; it SHALL activate the existing instance instead. The number of open transient tabs SHALL therefore be bounded by the number of registered transient page types.
...
- **WHEN** transient pages are triggered repeatedly across a settings session

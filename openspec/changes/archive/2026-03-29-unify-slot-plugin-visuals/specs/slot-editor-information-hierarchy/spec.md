...
#### Scenario: User selects a plugin type before choosing appearance polish
- **WHEN** the Create Slot picker presents available plugin types
- **THEN** plugin cards SHALL emphasize canonical iconography, naming, and selected-state feedback with a neutral baseline treatment rather than pre-assigning strong plugin-branded color as the dominant signal

#### Scenario: New slot draft has not received a user-selected color
- **WHEN** a user selects a plugin type and the slot draft is initialized
- **THEN** the draft SHALL remain visually valid without requiring a plugin-specific default color and SHALL preserve color as optional user-authored polish

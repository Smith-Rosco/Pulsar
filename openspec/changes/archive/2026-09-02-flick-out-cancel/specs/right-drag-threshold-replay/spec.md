...
- **AND** the menu is NOT in the flick-out escape state at release (flick-out cancel disabled, or the cursor is within the flick-out radius)
...
#### Scenario: Flick-out escape release cancels the gesture
- **WHEN** the menu was summoned by the gesture
- **AND** the menu is in the flick-out escape state at release
- **THEN** the menu SHALL close without executing any selection
- **AND** the release SHALL NOT be delivered to the source application

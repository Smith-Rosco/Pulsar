...
Keep an installed Pulsar build up to date from GitHub releases: detect the latest version through a resilient multi-tier check, download release assets with source failover and integrity verification, and surface the result in a non-intrusive, privacy-bounded About-page UI.

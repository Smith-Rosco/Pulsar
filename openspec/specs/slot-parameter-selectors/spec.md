# slot-parameter-selectors

## Purpose
Define selector-style parameter editing patterns for slot parameters backed by external pickers such as Secret Fill secrets.
...
#### Scenario: Secret Fill secret parameter appears in edit dialog
- **WHEN** a user opens slot configuration for a Secret Fill slot with a `secretId` parameter
...
#### Scenario: Secret Fill secret parameter appears in add dialog
- **WHEN** a user configures a new Secret Fill slot during add-slot flow
...
- **WHEN** a Secret Fill slot already contains a valid `secretId`
...
- **WHEN** a Secret Fill slot has no configured `secretId`
...
- **WHEN** the user chooses a secret from the Secret Fill selector dialog
...
- **WHEN** the user creates and confirms a new secret during Secret Fill selection

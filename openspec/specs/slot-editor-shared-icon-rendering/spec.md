...
#### Scenario: Plugin icon in picker has no outer container border
- **WHEN** the Create Slot picker renders any plugin type icon
- **THEN** the icon SHALL NOT be wrapped in an additional `Border` container, so `SlotOrb` is the sole visual frame for the icon

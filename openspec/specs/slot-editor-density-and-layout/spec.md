...
#### Scenario: Create Slot renders plugin-type choices at scale
- **WHEN** the Create Slot dialog renders a growing list of plugin types
- **THEN** the selection surface SHALL favor dense, scan-first items that keep icon, name, and selected state readable without requiring long vertically stacked cards for each option

#### Scenario: Create Slot groups plugin types into browseable categories
- **WHEN** the dialog presents plugin type categories or tabs for scanning
- **THEN** the category controls and card layout SHALL maintain consistent card width and a stable selection rhythm rather than allowing each option row to size itself from its full descriptive content
...
#### Scenario: Create Slot renders the full authoring flow
- **WHEN** the Create Slot dialog shows left-column selection, right-column configuration, preview context, and optional polish controls together
- **THEN** the layout SHALL use a small number of strong landmarks and SHALL rely on spacing and ordering before introducing explanatory headings for every subsection

## Purpose
Define the unified configuration reset behavior so resetting the app returns it to the same lifecycle state, onboarding posture, and smart-default flow as a fresh first launch while preserving a recoverable backup of the prior configuration.

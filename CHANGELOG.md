...
### Added
- **插件页「内置 / 外部」标签头不可点击的根因修复 + 全类守卫**（2026-09-12 用户报告）：设置 → 插件页两个 TabItem 标签头真实鼠标点击无效、标签永不切换。根因**不是遮挡**：`Themes/DialogTemplates.xaml` 的**隐式** `DataTemplate DataType="{x:Type sys:String}"`（为对话框长消息可滚动而设，经 `App.xaml` 全局合并）对全应用所有 ContentPresenter 呈现的字符串生效——TabItem.Header（一个普通 string）因此被渲染成「**可聚焦的 ScrollViewer**」。现代 WPF 的 TabItem 选中是**焦点驱动**（`OnMouseLeftButtonDown → SetFocus() → OnPreviewGotKeyboardFocus → IsSelected`），点击时键盘焦点被 header 里的 ScrollViewer 截走（E2E 真实点击 + 焦点探针实证：`NewFocus=ScrollViewer(chain=...ContentSite←TabItem)`，无 `SelectionChanged`），选中契约断裂。修复 = 模板根 ScrollViewer 加 `Focusable="False"`（渲染型文本不需要键盘焦点，滚轮滚动不依赖焦点，对话框消息体功能无损）；一处修复覆盖整个缺陷类（所有被该模板包裹的字符串渲染位点：Expander 头 / 菜单头等同时痊愈）。取证与验证全走运行时证据：`Pulsar.E2E` 真实鼠标点击回路（非低干扰模式保命中测试保真）修复前 **9 连红**、修复后 **2 连绿**（`artifacts/e2e-plugins-tabs`，前后焦点签名对照见 lesson）；build **0 警告 / 0 错误**，全量 **1637/1637**（基线 1636 + 1）。新增：两 TabItem 与外标签安装按钮、搜索框的稳定 `AutomationId`（`Pulsar.Settings.Plugins.TabBuiltIn / TabExternal / ExternalInstall / SearchBox`，沿用 `Pulsar.Settings.*` 惯例，供 E2E/QA 驱动）；静态守卫 `Pulsar.Tests/UI/ImplicitStringTemplateFocusGuardTests`（扫描全部源码 XAML：隐式 String DataTemplate 根元素必须 `Focusable="False"`，匹配集为空 fail-fast 防改名空转），**变异红证明**：移除属性 → 恰好 1 条守卫失败且报对文件与元素，恢复后复绿。→ `Docs/lessons/WPF_IMPLICIT_STRING_TEMPLATE_FOCUS_STEAL.md`
...
### Changed
- **文本型设置校验骨架收敛为单一 owner**（2026-09-12 架构审查 round 11）：`PluginSettingViewModel` 的 String / Secret / Path 三个子类 `Validate()` 曾各持一份同构的「required → min/max length（→ pattern）」阶梯——其中 String 与 Secret 连 resx 键组都逐字相同，`StringValue` / `SecretValue` / `PathValue` / `SelectedOption` 四个 getter 也是同一取值形态（`Value ?? Default ?? ""`）的复制。收敛为基类 `ValidateText(text, requiredKey, minLengthKey, maxLengthKey, checkPattern)`：结构只有一个 owner，类型间的真实分面差异（resx 键组、是否检查 Pattern）由调用方传参；`TextValue` 统一取值形态，四个 getter 收缩为别名。净删 39 行。新增守卫 `Pulsar.Tests/ViewModels/Settings/PluginSettingValidationTests` 7 例（String/Secret 键组一致、Path 用 Path 组键且不查 Pattern、骨架顺序 required > length > pattern、非法正则按无约束、Value 变更触发重校验、ResetToDefault 清除消息、TextValue 回退默认值）。**变异红证明**：短路 MinLength 检查 → 恰好 5 条失败且全部落在 MinLength 依赖链上，其余 5 条不波及；恢复后复绿。build 0/0，全量 **1636/1636**（基线 1626 + 10 新守卫）。
- **槽位编辑器「上下文 → 槽位源列表」映射收敛为单一 owner**（2026-09-12 架构审查 round 10）：`SlotEditorWorkspace` 的「Launcher→`Global.SwitchMode`（启动器即切换面）/ Global→`Global.CommandMode` / 其余上下文→各自 profile 的 `CommandMode`」这条规则曾**各写一遍**——加载选中上下文的 `OnCurrentContextChanged` 与统计条数的 `UpdateContextStats` 各持一份同构三分支，任一处漏改就会出现「列表有 N 个槽、徽标显示 0」。现收敛为私有方法 `ResolveSlotSourceList(ContextInfo)`，两处都经它取数；方法注释钉住「都必须经由」与 null 语义（= 该上下文当前没有槽位列表）。build 0/0，全量测试与基线一致。
...
### Removed
- **Settings 子系统死元素簇（架构审查 round 10，2026-09-12）**：8 块零引用成员逐个 grep 全仓（生产 + 测试 + Simulator + E2E）证实无消费者后删除，共 **−69 行、零新增**：`SettingsViewModel.TryDiscoverIconForProcess`（约 29 行，零调用方）、`PauseHotkeys` / `ResumeHotkeys`（零调用方）、`IsSettingsView` / `IsSlotsView`（全仓零读取、XAML 无绑定；连带删除 `OnSettingsShellPropertyChanged` 里对应的两行 `OnPropertyChanged`）、`AddSlot` + `AddSlotOfType`（`[RelayCommand]` 生成的 `AddSlotCommand` / `AddSlotOfTypeCommand` **全仓零绑定**，注释自称"legacy 兼容"的兜底从未有人用；XAML FAB 实际绑定的是 `AddSlotDialogCommand`）、`SetSlotAction` 转发包装（真正的实现在 `SlotEditorWorkspace.SetSlotAction`，测试钉的就是那个）、`PluginViewModel.ConfigService` 属性（零读取；全仓 `.ConfigService` 命中均为测试 fixture 的同名成员）。**红证明 = 编译器枚举**：删净后 build **0 警告 0 错误**，无任何残留引用；连带面逐个核对（`_hotkeyService` 仍被 `RebuildCache` 使用、`IconHelper` 仍被 `:946` 使用、`CurrentView` 仍被 `PageNavigatedTriggerHandler` 读取 → 均非连带死元素，保留）。build 0/0，全量 **1626/1626** 与基线一致（零测试引用 = 零调用方实证）。
- **教程步骤文本收敛到单一来源（架构审查候选 #5，2026-09-11）**：5 个 `TutorialSteps*.json` 曾同时写散文（`title`/`description`/`waitHintText`/`primaryButtonText`）与本地化键（`*Key`），而 `TutorialStepCard.RefreshStepContent` 运行时恒取键 —— `ILocalizationService` 缺键时返回键名本身、从不回退到散文，故那 **75 处散文是与 resx 值逐字重复的死副本**，全部删除（每文件 −15 行、零新增、JSON 语法校验通过）。随之零触发的 `TutorialStepLoader.ApplyConventionKeys` + `DeriveLocSuffix`（从 step id 约定推导键的兜底；JSON 已全部显式写 key）一并删除。**`GetFallbackSteps()` 保留**：它已走 `_loc[key]`，是「结构」的第三份而非文本死副本，且承担 JSON 缺失时的兜底职责。新增守卫 `Pulsar.Tests/Tutorial/TutorialStepLocalizationGuardTests` 4 例（散文不得回潮 / 每步必须声明 `titleKey`+`descriptionKey` / 所有 `*Key` 必须在双语 resx 存在）；`ScenarioStepLoaderTests` 两条「描述应提到 Macro / Browser Script」由钉 JSON 散文改为钉 resx 值（文本的唯一来源）。**红证明**：向测试实际读取的 bin 副本注入一行散文 → **恰好 1 条守卫失败**（`Steps_ShouldNotCarryProseCopies`），其余 3 条通过；恢复后复绿。build **0 警告 0 错误**，全量 **1626/1626**（1622 + 4 新守卫，精确对账）。
- **轮盘键盘导航整体移除**（2026-09-11 用户裁定）：轮盘只在快捷键**按住期间**存在，选择一律走指针（悬停命中 + 磁吸）；再叠一套键盘语义会造成两个"当前目标"来源。删除 `ViewModels/RadialKeyboardNavigator.cs`（86 行）及其测试文件（109 行）；`RadialMenuWindow.OnPreviewKeyDown` 收缩为只处理 `Esc`（= 取消，保留——与右键/点空白/看门狗互为冗余）。移除项：方向键选扇区（含双键对角）、数字键 1-9 直达、`Enter` 执行、`PgUp`/`PgDn` 翻页（翻页只剩鼠标滚轮）；随之 `MenuSession.HandlePagingKey` 与 `RadialMenuViewModel.HandlePagingKey`（PgUp/PgDn 专用适配器）成为零调用方，一并删除。**顺带清掉一个既有缺陷**：`OnPreviewKeyUp` 从未接线（全仓无 `KeyUp +=` 指向它）→ `_heldDirectionKeys` 永不移除 → 同一方向键在单次会话中只有首次生效、双键对角实际不可用。**连带损失已拦截**：被删测试文件里夹着 3 条与键盘无关的钉子（`SlotOrb.HoverEnterDuration ≤ 120ms` / `HoverReleaseDuration = 320ms` / `SlotViewModel.MaxMagneticSpeed ≥ 400px/s`），全仓仅此一处覆盖 → 迁入 `Pulsar.Tests/Views/Controls/SlotInteractionTimingTests.cs` 原样保留。**历史记录不改写**（`RELEASE_NOTES-1.14.0` 中"轮盘支持键盘导航"保留为 1.14.0 的既成事实）。测试对账：删除前 1643 → 删 24 条键盘用例 = 1619（实测）→ 迁回 3 条鼠标钉子 = **1622/1622**（实测）；build **0 警告 / 0 错误**。

### Fixed
- **插件页工具栏右缘溢出——窗口缩窄时 Refresh 按钮被裁出窗口**（2026-09-12 上一缺陷诊断顺带发现，用户裁定修复）：工具栏（页头下独立行）是**水平 `StackPanel`**——以无限宽测量子项、不收缩、不裁剪；设置窗口 `Width=1000 Height=700` **无 MinWidth**，缩窄时整行右溢窗口边缘（UIA 实证：1000×700 DIP 下 Refresh 右缘 x=2130 超窗口右界 2030 共 100 物理px，仅剩 ~20px 细条可见）。修复：StackPanel → Grid 五列（`Auto|Star|Auto|Auto|Auto`），**搜索框入 Star 列**（`MaxWidth=240` 封顶 / `MinWidth=140` / `HAlign=Left`）——宽度不足由弹性元素吸收，Auto 控件永不被推出可视区；XAML 注释钉死「此处禁用水平 StackPanel」及原因（防 e2ae5f7 式回归）。验证走**数值红绿对照**（E2E `dump` 步骤在 PASS 时也产出 UIA 树，`artifacts/e2e-plugins-tabs/toolbar-fix1/toolbar-uia-tree.txt`）：同窗口尺寸下 Refresh 右缘 2130 → **1992**（≤窗口 2030、恰贴内容区右缘），SearchBox 宽 360px → 222px（Star 列收缩生效）；E2E 全回路 15 步 PASS（含内置/外部标签切换回归）；build 0 警告 / 0 错误，全量 **1637/1637** 与基线一致。
- **插件设置对话框两处接缝缺陷**（2026-09-12 架构审查 round 11）：① **跨接缝泄漏**——`PluginSettingsDialogViewModel` 经 `_pluginViewModel.DialogService` 挖取对话框服务（该公开属性存在的唯一理由就是被挖），改为构造注入 `IDialogService?`（`PluginViewModel.Configure` 传入 `_dialogService`），公开属性删除；② **`async void OnSettingChanged`**——validate 段在 try/catch 之外（校验/本地化异常会穿透 WPF 消息循环崩进程），且 ProcessBlacklist 确认路径把两个 async void 当普通方法 fire-and-forget 调用、与后续 `LoadSettings` 重建存在竞态。拆为「事件薄壳（整体 try/catch，异常只记日志）+ `PersistSettingAsync`（可 await）」，两个调用点改 `await`，顺序与错误边界同时修好。守卫 `PluginSettingsDialogSeamTests` 3 例（构造签名必须含 `IDialogService` / `PluginViewModel` 不得有公开 `DialogService` / async void 方法清单反射钉 = `{ OnSettingChanged }` 且非 public）。**红证明**：改前窄跑恰好 2 条失败（构造签名 / 公开属性），改后复绿。build 0/0，全量 **1636/1636**。
- **插件详情对话框标题里从未出现插件名**（2026-09-11 做候选 #4 时发现，本日修）：`Dialog.PluginAnalyticsDetail.Title` 的 resx 双语值为 `Plugin Details` / `插件详情`（**无 `{0}` 占位符**），而调用点按格式串传入 `item.DisplayName` → `string.Format` 对无占位符的模板**静默丢弃**多余实参，插件名从未渲染。修复（取方案①）：双语值改为 `Plugin Details: {0}` / `插件详情：{0}`，`DialogCatalog` 目录行 `TitleIsFormat = true`，`SettingsAnalyticsPageViewModel` 补回实参。守卫双向：`DialogCatalogTests.TitleFormatFlag_ShouldMatchTheResxValue`（标记与 resx 值任一方向失配即失败）+ `ViewDetails_OpensDetailDialog_ForPlugin` 由 `It.IsAny<object[]>()` 改为钉「实参真的传出去」（长度 1 且非空白），防「传了但没人用」回潮。**用户可见文案变化**：插件详情对话框标题现在带插件名。build 0/0，全量 **1626/1626**。

...
- 定向验证：`AppStartupCoordinatorTests`（8，新增）+ `OnboardingVerificationTests`（11）= 19/19 通过（含 ADR-018 修复后的自愈契约反转断言）；全量 `dotnet test` 后台跑至 ~15 分钟无输出疑似挂起已终止，全量计数待重跑回填（此前基线 1037/1037）
